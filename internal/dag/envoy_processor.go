...
var _ Processor = &EnvoyProcessor{}

func (p *EnvoyProcessor) Run(dag *DAG, cache *KubernetesCache) {
	for _, e := range cache.envoys {
		envStatus, commit := status.EnvoyAccessor(&dag.StatusCache, e)
		validCondition := envStatus.ConditionFor(status.ValidCondition)
...
// buildEnvoy builds one Envoy record based on the corresponding CRD.
func (p *EnvoyProcessor) buildEnvoy(
	cache *KubernetesCache,
	env *contour_api_v1alpha1.Envoy,
	validCondition *contour_api_v1.DetailedCondition,
) *ExtensionCluster {
	tp, err := timeoutPolicy(env.Spec.TimeoutPolicy)
	if err != nil {
	...
	extension := ExtensionCluster{
		Name: ExtensionClusterName(k8s.NamespacedNameOf(env)),
		Upstream: ServiceCluster{
			ClusterName: path.Join(
				"extension",
				xds.ClusterLoadAssignmentName(k8s.NamespacedNameOf(env), ""),
			),
		},
		Protocol:           "h2",
	...
	lbPolicy := loadBalancerPolicy(env.Spec.LoadBalancerPolicy)
	switch lbPolicy {
	...
	if timeouts := env.Spec.TimeoutPolicy; timeouts != nil && timeouts.Idle != "" {
	...
	if env.Spec.Protocol != nil {
		extension.Protocol = stringOrDefault(*env.Spec.Protocol, extension.Protocol)
	}

	if v := env.Spec.UpstreamValidation; v != nil {
		if uv, err := cache.LookupUpstreamValidation(v, env.GetNamespace()); err != nil {
	...
	for _, target := range env.Spec.Services {
