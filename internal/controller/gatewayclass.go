...
// NewGatewayClassController creates the gatewayclass controller from mgr. The controller will be
// pre-configured to watch for GatewayClass objects.
func NewGatewayClassController(mgr manager.Manager, eventHandler cache.ResourceEventHandler, log logrus.FieldLogger) (controller.Controller, error) {
	...
	// Only enqueue GatewayClass objects that specify contour as the controller.
	if err := c.Watch(&source.Kind{Type: &gatewayapi_v1alpha1.GatewayClass{}}, r.enqueueRequestForGatewayClass()); err != nil {
		return nil, err
	}
...
// GatewayClass objects owned by the controller.
func (r *gatewayClassReconciler) enqueueRequestForGatewayClass() handler.EventHandler {
	return handler.EnqueueRequestsFromMapFunc(func(a client.Object) []reconcile.Request {
		...
		r.log.WithField("name", gc.Name).Info("gatewayclass not owned by contour, bypassing reconciliation")
		return []reconcile.Request{}
	})
}
...
func (r *gatewayClassReconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err := validation.GatewayClass(gc); err != nil {
		r.log.WithField("name", gc.Name).Error("invalid gatewayclass: ", err)
		valid = false
	}
	r.log.WithField("name", gc.Name).Info("valid gatewayclass")
